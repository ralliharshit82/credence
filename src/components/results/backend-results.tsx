...
export function BackendResults({ data, isDemo = false }: BackendResultsProps) {
  ...
  // Verdict Theme Helpers with rich ice-blue and tinted card gradients
    ...
    switch (level) {
      case 'HIGH_RISK':
        return {
          label: 'HIGH RISK',
          icon: ShieldAlert,
          badgeBg: 'bg-rose-600 text-white shadow-sm font-bold',
          scoreColor: 'text-rose-600',
          cardContainer: 'bg-gradient-to-br from-[#fff1f2] via-[#ffe4e6] to-[#fecdd3] border-2 border-rose-300 shadow-md shadow-rose-900/5',
          recBg: 'bg-white/90 border border-rose-300 text-rose-950 shadow-sm',
          recIconColor: 'text-rose-600',
          metaBg: 'bg-white/80 border border-rose-200',
        };
      case 'CAUTION':
        return {
          label: 'MEDIUM RISK',
          icon: AlertTriangle,
          badgeBg: 'bg-amber-500 text-white shadow-sm font-bold',
          scoreColor: 'text-amber-600',
          cardContainer: 'bg-gradient-to-br from-[#fffbeb] via-[#fef3c7] to-[#fde68a]/60 border-2 border-amber-300 shadow-md shadow-amber-900/5',
          recBg: 'bg-white/90 border border-amber-300 text-amber-950 shadow-sm',
          recIconColor: 'text-amber-600',
          metaBg: 'bg-white/80 border border-amber-200',
        };
      case 'LOWER_RISK':
      default:
        return {
          label: 'LOW RISK',
          icon: ShieldCheck,
          badgeBg: 'bg-emerald-600 text-white shadow-sm font-bold',
          scoreColor: 'text-emerald-600',
          cardContainer: 'bg-gradient-to-br from-[#f0fdf4] via-[#dcfce7] to-[#bbf7d0]/60 border-2 border-emerald-300 shadow-md shadow-emerald-900/5',
          recBg: 'bg-white/90 border border-emerald-300 text-emerald-950 shadow-sm',
          recIconColor: 'text-emerald-600',
          metaBg: 'bg-white/80 border border-emerald-200',
        };
    }
  };
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-200">
      ...
      {/* 1. MAIN VERDICT + SCORE + RECOMMENDATION CARD */}
      <div className={cn('rounded-3xl p-6 sm:p-8 text-left space-y-6', theme.cardContainer)}>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-900/10">
          <div className="space-y-1.5">
            <div className="flex items-center gap-2">
              <span className={cn('inline-flex items-center gap-1.5 rounded-full px-3.5 py-1 text-xs font-black tracking-wide', theme.badgeBg)}>
                <VerdictIcon className="h-4 w-4" />
                <span>{theme.label}</span>
              </span>
              {isDemo && (
                <span className="text-[11px] font-mono font-bold text-slate-700 bg-white/80 px-2.5 py-0.5 rounded-full border border-slate-300">
                  Demo Fixture
                </span>
              )}
            </div>
          ...
          {/* Score Box */}
          <div className={cn('flex items-center gap-4 px-6 py-3.5 rounded-2xl shrink-0 shadow-sm', theme.metaBg)}>
            <div>
              <div className={cn('text-3xl sm:text-4xl font-black font-mono', theme.scoreColor)}>
                {data.risk_score}
        ...
        <div className={cn('rounded-2xl p-4 sm:p-5 flex items-start gap-3.5', theme.recBg)}>
          <VerdictIcon className={cn('h-6 w-6 shrink-0 mt-0.5', theme.recIconColor)} />
          <div className="space-y-1">
            <span className="text-[10px] uppercase font-black tracking-wider opacity-75 block font-mono">
        ...
        <div className="flex items-center justify-between text-xs text-slate-700 pt-1 font-medium">
          <div className="flex items-center gap-2">
            <span className="font-mono text-[11px]">Domain: <strong className="text-slate-900">{data.categories.digital.domain}</strong></span>
            <span>•</span>
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 rounded-xl bg-white hover:bg-slate-50 border border-slate-300 px-3.5 py-1.5 text-xs text-slate-800 shrink-0 font-mono font-bold transition-all shadow-sm"
          >
      ...
      {/* 2. 🔊 LISTEN TO YOUR RESULT CARD (RICH ICE-BLUE) */}
      {speechSupported && (
        <div className="rounded-3xl border-2 border-blue-300 bg-gradient-to-r from-[#dbeafe] via-[#e1effe] to-[#dbeafe] p-5 sm:p-6 text-left space-y-3 shadow-md shadow-blue-900/5">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            
            {/* Audio Header */}
            <div className="flex items-center gap-3.5">
              <div className={cn(
                'h-11 w-11 rounded-2xl flex items-center justify-center border-2 transition-all shadow-sm',
                isSpeaking ? 'bg-blue-600 text-white border-blue-600 animate-pulse' : 'bg-white border-blue-300 text-blue-600'
              )}>
              ...
              <div>
                <h3 className="text-sm sm:text-base font-bold text-slate-900 flex items-center gap-2">
                  ...
                  {isSpeaking && (
                    <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-mono font-bold bg-blue-600 text-white animate-pulse">
                      Playing Audio...
                    </span>
                  )}
                </h3>
            ...
            <div className="flex items-center gap-2.5 shrink-0">
              ...
              {/* Language Selector */}
              <div className="flex items-center rounded-2xl bg-white border border-blue-300 p-1 text-xs font-bold shadow-sm">
                <button
                  type="button"
                  onClick={() => handleLanguageChange('en')}
                  className={cn(
                    'px-3.5 py-1.5 rounded-xl transition-all',
                    selectedLang === 'en' ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-700 hover:text-blue-700'
                  )}
                >
                  English
                </button>
                <button
                  ...
                  className={cn(
                    'px-3.5 py-1.5 rounded-xl transition-all',
                    selectedLang === 'hi' ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-700 hover:text-blue-700'
                  )}
                >
                  हिन्दी (Hindi)
                </button>
              ...
              <button
                ...
                className={cn(
                  'flex items-center gap-2 rounded-2xl px-5 py-2.5 text-xs font-black transition-all text-white shadow-md',
                  isSpeaking ? 'bg-amber-600 hover:bg-amber-700 shadow-amber-600/30' : 'bg-blue-600 hover:bg-blue-700 shadow-blue-600/30'
                )}
              >
                {isSpeaking ? (
                  <>
      ...
      {/* 3. 🔍 PROMINENT "WHY THIS VERDICT?" EVIDENCE CARD (ICE-BLUE CONTAINER) */}
      <div className="rounded-3xl border-2 border-blue-200/90 bg-gradient-to-br from-[#eaf2fe] via-[#edf4fe] to-[#e4effd] p-6 sm:p-7 text-left space-y-4 shadow-md shadow-blue-900/5">
        <div className="flex items-center justify-between pb-3.5 border-b border-blue-200">
          <div className="flex items-center gap-3">
            <div className="h-9 w-9 rounded-xl bg-blue-600 text-white flex items-center justify-center shadow-sm">
              <Info className="h-5 w-5" />
            </div>
            <div>
              <h3 className="text-base sm:text-lg font-bold text-slate-900">Why This Verdict?</h3>
              <p className="text-xs text-slate-600 font-medium">Key evidence signals evaluated by the Credence risk engine.</p>
            </div>
          </div>

          <span className="text-xs font-mono font-bold text-blue-800 bg-white px-3 py-1 rounded-full border border-blue-300 shadow-sm">
        ...
        {data.signals.length === 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3.5 pt-1">
            <div className="rounded-2xl bg-white border border-emerald-200 p-4 space-y-1 shadow-sm">
          ...
          <div className="space-y-3">
            {data.signals.slice(0, 4).map((sig, idx) => {
              ...
              return (
                <div
                  key={idx}
                  className="rounded-2xl bg-white border border-blue-200/90 p-4 sm:p-5 space-y-2 shadow-sm"
                >
                  <div className="flex items-center justify-between gap-2">
                    <div className="flex items-center gap-2.5 min-w-0">
                      <span className={cn(
                        'px-2.5 py-0.5 rounded-full text-[10px] font-mono font-bold uppercase tracking-wider border shrink-0',
      ...
      <div className="space-y-4 pt-2">
        ...
        {/* Navigation Tabs */}
        <div className="flex items-center gap-2 border-b border-blue-200 pb-3 overflow-x-auto">
          {[
            { id: 'signals', label: 'All Evidence Signals', count: data.signals.length },
            { id: 'identity', label: 'Claim vs Reality' },
            { id: 'reputation', label: 'Threat Intel & Brand', highlight: Boolean(data.categories.reputation?.threat_record || data.categories.reputation?.brand_impersonation) },
            ...
            return (
              <button
                ...
                className={cn(
                  'flex items-center gap-2 rounded-xl px-4 py-2 text-xs font-bold whitespace-nowrap transition-all shadow-sm',
                  isActive
                    ? 'bg-blue-600 text-white shadow-md shadow-blue-600/20'
                    : 'bg-white text-slate-700 hover:text-blue-700 hover:bg-blue-50 border border-blue-200'
                )}
              >
                <span>{tab.label}</span>
                {tab.count !== undefined && tab.count > 0 && (
                  <span className={cn('rounded-full px-2 py-0.2 text-[10px] font-mono', isActive ? 'bg-blue-800 text-white' : 'bg-blue-100 text-blue-800 font-bold')}>
                    {tab.count}
                  </span>
                )}
                {tab.highlight && (
        ...
        {activeTab === 'signals' && (
          <div className="space-y-3">
            {data.signals.length === 0 ? (
              <div className="rounded-3xl bg-white border border-blue-200 p-6 text-center space-y-1 shadow-sm">
              ...
              data.signals.map((sig, idx) => {
                ...
                return (
                  <div
                    key={idx}
                    className="rounded-2xl border border-blue-200 bg-white overflow-hidden text-left shadow-sm"
                  >
                    <button
                      type="button"
                      onClick={() => setExpandedSignal(isExpanded ? null : idx)}
                      className="w-full flex items-center justify-between p-4 sm:p-5 text-left gap-3 hover:bg-blue-50/50 transition-colors"
                    >
                    ...
                    {isExpanded && (
                      <div className="border-t border-blue-100 bg-[#f4f8fe] p-4 sm:p-5 space-y-4 text-xs">
                        ...
                        <div className="space-y-1">
                          ...
                          <p className="text-slate-800 leading-relaxed font-medium text-xs sm:text-sm">
                            {sig.explanation}
                          </p>
                        </div>

                        {/* FORMATTED EVIDENCE BREAKDOWN */}
                        <div className="rounded-2xl bg-white border border-blue-200 p-4 space-y-2 shadow-sm">
                          <div className="flex items-center justify-between">
                            <span className="text-[10px] font-mono uppercase text-blue-700 font-bold flex items-center gap-1.5">
                            ...
                            <button
                              ...
                              className="text-[10px] font-mono font-bold text-blue-600 hover:text-blue-800 underline"
                            >
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 pt-1 text-slate-700">
                            {Object.entries(sig.evidence)
                              ...
                              .map(([k, v]) => (
                                <div key={k} className="flex flex-col bg-[#f0f6ff] rounded-xl p-2.5 border border-blue-200">
                          ...
                          {/* Raw JSON Accordion */}
                          {isJsonVisible && (
                            <pre className="mt-2 p-3 rounded-xl bg-slate-900 font-mono text-[11px] text-slate-200 overflow-x-auto whitespace-pre-wrap break-all border border-slate-800">
        ...
        {activeTab === 'identity' && (
          <div className="rounded-3xl bg-white border border-blue-200 p-6 space-y-4 text-left shadow-sm">
            <h3 className="text-base font-bold text-slate-900">Identity & Association Audit</h3>
            
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5 text-xs">
              <div className="rounded-2xl bg-[#f0f6ff] border border-blue-200 p-4 space-y-1">
                <span className="text-[10px] font-mono text-slate-500 uppercase font-bold">1. Claimed / Extracted Lender</span>
                <div className="font-bold text-slate-900 text-sm">{data.identity.claimed_lender || 'None declared on page'}</div>
        ...
        {activeTab === 'reputation' && (
          <div className="rounded-3xl bg-white border border-blue-200 p-6 space-y-4 text-left text-xs shadow-sm">
            <h3 className="text-base font-bold text-slate-900">Brand Impersonation & Threat Intelligence</h3>

            <div className="rounded-2xl bg-[#eaf2fe] border border-blue-200 p-3.5 text-slate-700 text-xs">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
              ...
              <div className="rounded-2xl bg-[#f0f6ff] border border-blue-200 p-4 space-y-2">
                <span className="text-[10px] font-mono uppercase text-blue-700 font-bold flex items-center gap-1">
                ...
                {data.categories.reputation?.brand_impersonation?.detected ? (
                  <div className="space-y-1.5 text-slate-800">
                    ...
                    <div>Official Domain: <code className="text-emerald-700 font-bold bg-white px-2 py-0.5 rounded border border-emerald-200">{data.categories.reputation.brand_impersonation.expected_official_domain}</code></div>
                    <p className="text-rose-700 font-bold text-xs pt-1">
                      {data.categories.reputation.brand_impersonation.similarity_reason}
                    </p>
                  </div>
                ) : (
                  <p className="text-slate-600 font-medium">No banking or NBFC brand impersonation detected in domain or subdomains.</p>
                )}
              </div>
              ...
              <div className="rounded-2xl bg-[#f0f6ff] border border-blue-200 p-4 space-y-2">
                ...
                {data.categories.reputation?.threat_record ? (
                  <div className="space-y-1.5 text-slate-800">
                    <div>Status: <span className="font-mono font-bold text-rose-700 bg-white px-2 py-0.5 rounded border border-rose-200">{data.categories.reputation.threat_record.status}</span></div>
                  ...
                  <p className="text-slate-600 font-medium">No documented threat intelligence flags found for this target.</p>
                )}
              </div>
            </div>
          </div>
        ...
        {activeTab === 'transparency' && (
          <div className="rounded-3xl bg-white border border-blue-200 p-6 space-y-4 text-left text-xs shadow-sm">
            <h3 className="text-base font-bold text-slate-900">Transparency Signals Audit</h3>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3.5">
              {[
                ...
                return (
                  <div key={item.key} className="rounded-2xl bg-[#f0f6ff] border border-blue-200 p-4 space-y-1.5 shadow-sm">
                    <div className="flex items-center justify-between">
                      ...
                      {isFound ? (
                        ...
                        <span className="text-[11px] text-rose-800 font-bold flex items-center gap-1 bg-rose-50 px-2 py-0.5 rounded-md border border-rose-200">
        ...
        {activeTab === 'regulatory' && (
          <div className="rounded-3xl bg-white border border-blue-200 p-6 space-y-4 text-left text-xs shadow-sm">
            <h3 className="text-base font-bold text-slate-900">Regulatory Verification</h3>

            <div className="rounded-2xl bg-amber-50 border border-amber-200 p-3.5 text-amber-900 text-xs font-medium">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
              <div className="rounded-2xl bg-[#f0f6ff] border border-blue-200 p-4 space-y-1.5">
                ...
                {data.categories.regulatory.claims.length > 0 ? (
                  <ul className="list-disc list-inside text-slate-800 space-y-1 font-medium">
                    {data.categories.regulatory.claims.map((c, i) => (
                      <li key={i}>{c}</li>
                    ))}
                  </ul>
                ) : (
                  <p className="text-slate-600">No explicit regulatory keywords found.</p>
                )}
              </div>
              ...
              <div className="rounded-2xl bg-[#f0f6ff] border border-blue-200 p-4 space-y-1.5">
                ...
                {data.categories.regulatory.record ? (
                  ...
                  <p className="text-slate-600 font-medium">No registered lender entity matched.</p>
                )}
              </div>
            </div>
          </div>
        ...
        {activeTab === 'forensics' && (
          <div className="rounded-3xl bg-white border border-blue-200 p-6 space-y-4 text-left text-xs shadow-sm">
            <h3 className="text-base font-bold text-slate-900">Digital Forensics</h3>
            
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3.5">
              <div className="rounded-2xl bg-[#f0f6ff] border border-blue-200 p-3.5 space-y-1">
        ...
        {activeTab === 'permissions' && (
          <div className="rounded-3xl bg-white border border-blue-200 p-6 space-y-4 text-left text-xs shadow-sm">
            <h3 className="text-base font-bold text-slate-900">Mobile Permissions & Language Patterns</h3>
            ...
            <div className="space-y-3.5">
              <div>
                ...
                {data.categories.permissions?.findings?.length === 0 ? (
                  <p className="text-slate-600 font-medium">No disproportionate runtime permissions declared.</p>
                ) : (
                  <div className="space-y-2">
                    {data.categories.permissions.findings.map((f, i) => (
                      <div key={i} className="rounded-2xl bg-[#f0f6ff] border border-blue-200 p-3.5 space-y-1">
                        <div className="flex items-center justify-between">
                          <strong className="text-slate-900 font-mono uppercase">{f.permission}</strong>
                          <span className={cn('text-[10px] font-mono font-bold px-2 py-0.5 rounded-full', f.severity === 'HIGH' ? 'bg-rose-100 text-rose-800' : 'bg-amber-100 text-amber-800')}>
                            {f.severity}
                          </span>
                        </div>
              ...
              <div className="pt-3 border-t border-blue-100">
                ...
                {data.categories.language?.detected_patterns?.length === 0 ? (
                  <p className="text-slate-600 font-medium">No aggressive predatory phrasing detected.</p>
                ) : (
                  <div className="flex flex-wrap gap-2">
                    {data.categories.language.detected_patterns.map((p, i) => (
                      <span key={i} className="rounded-xl bg-rose-50 border border-rose-200 px-3 py-1 text-rose-800 text-xs font-mono font-bold">
                        {p}
                      </span>
                    ))}
                  </div>
      ...
      {/* Bottom Disclaimer */}
      <div className="text-center text-[11px] text-slate-600 pt-2 font-medium">
